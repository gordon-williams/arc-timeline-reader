# Acknowledgements

Arc Timeline Diary Reader uses the following open source libraries and services:

## JavaScript Libraries

### Leaflet
- **Version:** 1.9.4
- **License:** BSD 2-Clause
- **Website:** https://leafletjs.com/
- **Description:** Interactive map library

### Leaflet.markercluster
- **Version:** 1.5.3
- **License:** MIT
- **Website:** https://github.com/Leaflet/Leaflet.markercluster
- **Description:** Marker clustering plugin for Leaflet

### Marked
- **License:** MIT
- **Website:** https://marked.js.org/
- **Description:** Markdown parser and compiler

### Chart.js
- **Version:** 4.4.1
- **License:** MIT
- **Website:** https://www.chartjs.org/
- **Description:** Charts and graphs for the Analysis panel

### Pako
- **Version:** 2.1.0
- **License:** MIT
- **Website:** https://github.com/nodeca/pako
- **Description:** Zlib compression for data handling

## Map Tile Providers

### CARTO (Street Map)
- **Website:** https://carto.com/
- **Attribution:** © OpenStreetMap contributors © CARTO
- **Description:** Light basemap tiles for street view

### CyclOSM (Cycle Map)
- **Website:** https://www.cyclosm.org/
- **Attribution:** © OpenStreetMap contributors, CyclOSM
- **Description:** Bicycle-focused map tiles

### Esri World Imagery (Satellite)
- **Website:** https://www.esri.com/
- **Attribution:** © Esri, Maxar, Earthstar Geographics
- **Description:** Satellite imagery tiles

## Data Services

### OpenStreetMap
- **License:** ODbL (Open Database License)
- **Website:** https://www.openstreetmap.org/
- **Description:** Map data foundation for street and cycle maps

### Nominatim
- **Website:** https://nominatim.org/
- **Description:** Geocoding service for location search (OpenStreetMap)
- **Usage Policy:** https://operations.osmfoundation.org/policies/nominatim/

## Data Source

### Arc Timeline
- **Website:** https://www.bigpaua.com/arc/
- **Developer:** Big Paua
- **Description:** iOS app that records location history. This reader imports Arc Timeline JSON export files.

## Development

### Claude Code
- **Website:** https://claude.ai/
- **Developer:** Anthropic
- **Description:** AI pair programming assistant used in the development of this application

---

## License

Arc Timeline Diary Reader is released under the MIT License.

Copyright © 2025–2026 Gordon Williams
