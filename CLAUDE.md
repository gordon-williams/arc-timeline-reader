...
| 1.2 | `arc-sprites-data.js` | `window.__ARC_SPRITES_DATA__` | External sprite data as a JS global. Loaded via `<script>` tag before `arc-sprites.js` — works on `file://` protocol where fetch/XHR fail. Generated by Sprite Editor export. |
| 1.5 | `arc-sprites.js` | `window.ArcSprites` | External sprite/icon data loader. Reads from `window.__ARC_SPRITES_DATA__` (set by `arc-sprites-data.js`), falls back to hardcoded defaults. Exposes accessor API, injects dynamic CSS for replay marker colours. |
...
| `sprite-viewer.html` | Sprite editor — tile catalogue grid with right sidebar editor. Light/dark theme, search filter, SVG staging/analysis, colour pickers, import/export `arc-sprites-data.js`. |

### Data Files

| File | Role |
|------|------|
| `arc-sprites.json` | External sprite data — SVGs, emoji, route colours, replay colours for all activity types. Used by Sprite Editor for import/export. Not loaded directly by the app at runtime. |
| `arc-sprites-data.js` | JS wrapper around sprite data (`window.__ARC_SPRITES_DATA__ = {...}`). Loaded by app via `<script>` tag — the only method that works on `file://` protocol where fetch/XHR fail. Generated by Sprite Editor export. |

### Photo Server (`photo-server/`)

| File | Role |
|------|------|
| `server.js` | macOS photo server — reads Apple Photos SQLite database, serves thumbnails/full-res via REST API. Uses `photo-fetch` Swift helper for iCloud downloads. |
| `server-windows.js` | Windows photo server — reads iCloud for Windows flat folder, extracts EXIF metadata with `exifr`. Same REST API as macOS server. |
| `Start Photo Server (Windows).bat` | Windows launcher — checks Node.js, installs deps, starts `server-windows.js`. |
...
- **External sprite overlay** — `arc-sprites-data.js` provides SVGs, emoji, and colours for activities via a `<script>` tag (the only loading method that works on `file://` protocol). `arc-sprites.js` reads from `window.__ARC_SPRITES_DATA__` and exposes the `window.ArcSprites` API. All consumers (`replay.js`, `app.js`, `analysis.html`, `analysis-ai.js`) read from ArcSprites first, falling back to their inline hardcoded values. The app works identically if the data file is missing.
